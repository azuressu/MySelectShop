...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT) // 서버의 POST를 랜덤으로 설정
@TestInstance(TestInstance.Lifecycle.PER_CLASS) // 테스트 인스턴스의 생성 단위를 클래스로 변경 - 클래스 내부에 선언해둔 필드를 공유할 수 있음
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
public class ProductServiceIntegrationTest {

    /* 각각의 테스트는 서로 영향을 받지 않기 위해서 따로 돌아감 - 같이 돌면 영향을 받으니까 */
    /* Spring boot가 붙으면 Ioc/DI를 사용할 수 있음 */
    ...
    void test1() {
        ...
        ProductRequestDto requestDto = new ProductRequestDto(
                title, imageUrl, linkUrl, lPrice
        );

        // 신규 상품을 등록할 때 user가 필요해서 만들어줌
    ...
    void test3() {
        ...
        // then
        // 1. 전체 상품에서 테스트에 의해 생성된 제품 찾아오기 (상품의 id로 찾음)
